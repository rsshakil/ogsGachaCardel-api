...
exports.handler = async (event) => {
	...
	try {
		...
		const user_data_sql = `SELECT userSMSFlag, userSMSTelNoFormat, userSMSTelLanguageCCValue FROM User WHERE userId = ? LIMIT 0, 1`;
		const [user_data_result] = await mysql_con.query(user_data_sql, [userId, userId]);

		if (Array.isArray(user_data_result) && user_data_result.length > 0) {
			const { userSMSFlag, userSMSTelNoFormat, userSMSTelLanguageCCValue } = user_data_result[0] || {};

			//For SMS verified user
			if (userSMSFlag == 1) {
				//Send 6 digit OTP
				...
				if (duplicatedQueryResult && duplicatedQueryResult.length >= 1) {
					...
					if (errorFlag) {
						// Insert SMS history
						await mysql_con.execute(user_sms_insert_sql, [otp, expireDate, userId, phoneNumber, userSMSTelLanguageCCValue, 2, 2, sourceIP, nowUnixTimestamp])
				...
				//Update DB
				const user_update_sql = `UPDATE User SET userPaymentSMSToken = ?, userPaymentSMSCheckExpiredAt = ? WHERE userId = ?`;
				await mysql_con.execute(user_update_sql, [otp, expireDate, userId]);

				// Insert SMS history
				await mysql_con.execute(user_sms_insert_sql, [otp, expireDate, userId, phoneNumber, userSMSTelLanguageCCValue, 2, 1, sourceIP, nowUnixTimestamp])

				const user_shipping_address_sql = `
				SELECT 
					userShippingId,
					userShippingName,
					userShippingZipcode,
					...
					CASE WHEN userShippingPriorityFlag = 1 THEN 1 ELSE 0 END AS isItemSelected,
					userShippingMemo
				FROM UserShipping 
				WHERE userShippingUserId = ?`;

				const [user_shipping_address_data] = await mysql_con.query(user_shipping_address_sql, [userId]);

				const response = {
					smsAuthNo: userSMSTelNoFormat,
					myShippingAddress: user_shipping_address_data
				}

				return getResponse(response, 200);
			}

			//For SMS not authenticated user
			throw new Error(403);
		}
