...
exports.handler = async (event) => {
    ...
    try {
        ...
        const languageSql = `SELECT 
                languageId
            FROM Language 
            WHERE languageCode = ?`; 
        const [languageData] = await mysql_con.query(languageSql, [languageCode]);
        const languageId = languageData[0].languageId;
        const update_sql = `UPDATE User SET userLanguageId = ? WHERE userId = ?`;
        const sql_param = [
            languageId,
            userId
        ];
        const [query_result] = await mysql_con.execute(update_sql, sql_param);
        await cluster.set("user:" + ENVID + ":" + userId + ":language", languageId)
