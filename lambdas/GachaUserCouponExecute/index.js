...
exports.handler = async (event, context, callback) => {
    ...
    try {
        ...
        //1. check is coupon available
        const coupon_used_count_subquery = `SELECT COUNT(userCouponId) AS useCount FROM UserCoupon WHERE userCouponCouponId = couponId`;
        ...
        const coupon_query = `
            SELECT 
                ...
                couponName
            FROM Coupon 
            WHERE couponCode = ? AND couponStatus = ? AND couponStartDate <= ? AND couponEndDate >= ? AND couponLimitCount > (${coupon_used_count_subquery})
            LIMIT 0, 1`;

        console.log('coupon_query >>>', coupon_query)
        console.log('coupon_query params >>>', [couponCode, 1, now, now])
        const [coupon_result] = await mysql_con.query(coupon_query, [couponCode.trim(), 1, now, now]);

        if (coupon_result.length > 0) {
            const { couponId, couponPoint, couponName } = coupon_result[0];

            //2. check this user already use this coupon earlier or not
            const user_coupon_use_count_query = `SELECT COUNT(userCouponId) AS useCount FROM UserCoupon WHERE  userCouponCouponId = ? AND userCouponUserId = ?`;
            const [user_coupon_use_count_result] = await mysql_con.query(user_coupon_use_count_query, [couponId, userId]);

            //Coupon Available to use
            if (user_coupon_use_count_result.length > 0 && user_coupon_use_count_result[0].useCount < 1) {
                await mysql_con.beginTransaction();

                //Insert new record => UserCoupon
                const user_coupon_insert_query = `INSERT INTO UserCoupon(userCouponUserId, userCouponCouponId, userCouponCreatedAt) VALUES (?, ?, ?)`;
                await mysql_con.query(user_coupon_insert_query, [userId, couponId, now]);
                ...
                /*
                                if (user_point_result.length > 0) {
                                    ...
                                    await mysql_con.query(user_pont_update_query, [couponPoint, userId]);
                                }
                                //Create new point record
                                    ...
                                    await mysql_con.query(user_pont_insert_query, [userId, couponPoint]);
                                }
                */
                //SQS Processing
                const params = {
                    MessageBody: JSON.stringify({ userId: userId, couponId, point: couponPoint, detailStatus: 7, executeAt: Math.floor(new Date().getTime() / 1000) }),
                    QueueUrl: queueUrl,
                ...
                await cluster.incrby(redisKeyUserPoint, couponPoint);

                await mysql_con.commit();

                return getResponse({ message: 'success', couponCode: couponCode.trim(), couponName: couponName, couponPoint: couponPoint }, 200);

            } else {
                //Coupon not Available to use
                return getResponse({ message: 'This coupon has already taken or expired1' }, 400);
            }
        }
        else {
            //Coupon not Available to use
            return getResponse({ message: 'This coupon has already taken or expired2' }, 400);
        }
